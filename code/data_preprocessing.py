...
def remove_ambiguous(dataframe):
    """Remove ambigous rows. Ambiguous rows are rows in which the orginal evaluation differs from the coders' average evaluation.
    Args:
...
def remove_similar(dataframe, cutoff=0.8):
    ...
    If the highest similarity to all other remaining messages is greater than a given cutoff, this row is removed, starting from first row.
    Never remove similar rows with different labels. Print deleted messages.
...
def add_similarity_score(df_similar, df_compare):
    """For each row in df_similar, add a similarity score that refers to the most similar message in df_compare for a given label.
    Similarity is evaluated using the difflib.SequenceMatcher()

    Args:
        df_similar (pandas dataframe): data to be transformed by adding a column, containing strings in the column 'text'
        df_compare (pandas dataframe): data to be compared, contains strings in the column 'text'
