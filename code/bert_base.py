...
import random
import time
from transformers import AutoTokenizer, BertForSequenceClassification, get_linear_schedule_with_warmup, BertTokenizer, BertConfig
...
# def preprocessing(dataset, tokenizer = AutoTokenizer): 
#     # Tokenize all of the sentences and map the tokens to thier word IDs.
#     sentences = list(dataset['text'])
#     labels = dataset['target_orig']
#     print('loading Bert tokenizer...')
#     tokenizer = tokenizer.from_pretrained('bert-base-uncased')
#     encoded_input = tokenizer(sentences, padding=True, return_tensors='pt') # dict with keys 'input_ids, token_type_ids', 'attention_mask'
...
def training(train_dataloader, val_dataloader, args, device): 
    ...
    model = BertForSequenceClassification.from_pretrained(
        "ishan/bert-base-uncased-mnli", classifier_dropout = args['classifier_dropout']
    )
    if args['reinit_layers']>0:
        reinit_layers(model, args)
    model.cuda()
...
def reinit_layers(model, args): 
    encoder_temp = getattr(model, args['model_type']) #encoder_temp = BertModel
    if args['model_type'] in ["bert", "roberta"]: #apparently, electra does not have a pooler 
