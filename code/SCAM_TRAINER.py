...
class ScamTrainer:
    ...
    def _preprocessing(self, dataset, feature_col = 'text', target_col = 'target_orig'):
        assert isinstance(dataset, pd.DataFrame), 'dataset must be a pandas DataFrame.'
    ...
    def _init_model(self): 
        if self.model_type == 'bert': 
            ...
            if not self.int_task: 
                self.model = RobertaForSequenceClassification.from_pretrained('roberta-base', classifier_dropout = self.classifier_dropout)
                print('Roberta base initialized and weights loaded..')
            else: 
                self.model = RobertaForSequenceClassification.from_pretrained('textattack/roberta-base-MNLI', classifier_dropout = self.classifier_dropout)
    ...
    def fit(self, train_dataset, val_dataset, feature_col = 'text', target_col = 'target_orig', seed = 100): 
        assert isinstance(seed, (int, list))
        if isinstance(seed, int): 
            seed = [seed]
        for random_seed in seed: 
            train_tensor = self._preprocessing(train_dataset, feature_col=feature_col, target_col=target_col)
            val_tensor = self._preprocessing(val_dataset, feature_col=feature_col, target_col=target_col)
            ...
            train_dataloader, val_dataloader = self._dataloaders(train_tensor, val_tensor, random_seed)
            ...
            for epoch_i in range(0, epochs):
                ...
                for step, batch in enumerate(train_dataloader): #len(train_dataloader) = len(train_dataset)//16
                    ...
                    total_train_accuracy += self._flat_accuracy(logits, label_ids)
                    loss.backward()
            ...
            self._save_training_summary(random_seed, avg_train_loss, avg_train_accuracy, avg_val_loss, avg_val_accuracy)
               
    def _save_training_summary(self, seed, avg_train_loss, avg_train_accuracy, avg_val_loss, avg_val_accuracy): 
        training_stats = {
            'seed': [seed],
        ...
        if not os.path.exists(output_dir):
            ...
            training_summary.to_excel(os.path.join(output_dir, 'training_summary.xlsx'), index=False)
            with open(os.path.join(output_dir, 'training_arguments.json'), 'w') as file:
                json.dump(self.args, file)
        else: 
            df_exist = pd.read_excel(os.path.join(output_dir, 'training_summary.xlsx'))
            df_exist=pd.concat([df_exist, training_summary]).reset_index(drop=True)
            df_exist.to_excel(os.path.join(output_dir, 'training_summary.xlsx'), index=False) #mix only different seeds, not arguments
