...
def augment_text(dataset, augmentation_type = 'contextual', samples_0 = 0, samples_1 = 0, aug_p = 0.2, seed = 10):
    ...
    if not augmentation_type in ['contextual', 'synonym_dict']: 
        raise ValueError('augmentation_type must be "contextual" or "synonym_dict"')
    ...
    if augmentation_type == 'contextual': 
        aug = naw.ContextualWordEmbsAug(model_path='bert-base-uncased', action="substitute", aug_p=aug_p)
    else: 
